...
$(() => {
  ...
  const sortKey = (element, column) => {
    const $cell = $(element).children("td").eq(column);
    const sortText = $cell.find("span.sort-key").text();
  ...
  $("#t-1")
    ...
    .on("click", (e) => {
      ...
      $("#t-1")
        .find("tbody > tr")
        .each((i, element) => {
          $(element).data("sortKey", sortKey(element, column));
        })
        .get()
