...
(($) => {
  $.cssHooks.glowColor = {
    set(elem, value) {
      elem.style.textShadow = value == "none" ? "" : `0 0 2px ${value}`;
    },
  };
})(jQuery);
...
(($) => {
  const buildAuthors = (row, separator = ", ") =>
    row.authors.map((a) => `${a.first_name} ${a.last_name}`).join(separator);
      ...
      $("#t-3")
        ...
        .on("click", (e) => {
          ...
          const content = buildRows(
            prepRows(json).sort((a, b) =>
              comparator(a[sortKey], b[sortKey], sortDirection)
            )
          );

          $("#t-3").find("tbody").html(content);
